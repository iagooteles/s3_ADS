-- Observação: O Mysql limita a quantidade de updates ou deletes em uma tabela. 
-- Isso inviabiliza a deleção ou atualização de todos os registros de uma só vez.
-- Para desabilitar essa limitação execute o comando "SET SQL_SAFE_UPDATES = 0;".
